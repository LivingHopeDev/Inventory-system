...
export class OrderService {
    public async createOrderFromCartItems(userId: string, cartItemIds: string[], shippingAddress: string, billingAddress: string): Promise<{ message: string, data: Partial<Order> }> {
    ...
    public async getAllOrders(userId: string): Promise<{ message: string, data: Partial<Order>[] }> {
    ...
    public async getOrderById(orderId: string, userId: string): Promise<{ message: string, data: Partial<Order> }> {
    ...
    public async updateOrder(orderId: string, userId: string, payload: Partial<Order>): Promise<{ message: string, data: Partial<Order> }> {
        const order = await prismaClient.order.findFirst({
            where: { id: orderId, userId },
        });
    ...
    public async cancelOrder(orderId: string, userId: string): Promise<{ message: string, data: Partial<Order> }> {
        const order = await prismaClient.order.findFirst({
            where: {
                id: orderId,
                userId,
        ...
        if (!order) {
            throw new ResourceNotFound(`Order with ID ${orderId} not found or out for delivery`);
        }
